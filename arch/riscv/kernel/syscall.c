...
struct ret_info syscall(uint64_t syscall_num, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5) {
    ...
    case SYS_MMAP: {
        ...
        vma->vm_start = (unsigned long)arg0;
        vma->vm_end = (unsigned long)(arg0+arg1);
        // 2.2 set the vm_flags to arg2
        vma->vm_flags = (unsigned long)arg2;
        // 2.3 set the mapped flag to 0
        vma->mapped = 0;
        // 3. add the vma struct to the mm_struct's vma list
        list_add(&(vma->vm_list),&(current->mm.vm->vm_list)); // parameter 
        // return the vm_start
        ret.a0 = vma->vm_start;
        break;
    ...
    case SYS_MUNMAP: {
        ...
        list_for_each_entry(vma, &current->mm.vm->vm_list, vm_list){
            if(vma->vm_start == arg0 && vma->vm_end == arg0 + arg1){
                if(vma->mapped == 1){
                    uint64_t entry = get_pte((current->satp & ((1ULL << 44) - 1)) << 12,vma->vm_start);//
                    free_pages((entry >> 10) << 12);
                }
